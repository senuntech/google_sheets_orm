...
void main() {
  ...
  setUp(() async {
    ...
    when(
      () => mockFiles.list(q: any(named: 'q')),
    ).thenAnswer((_) async => fileList);

    final existingSpreadsheet = sheets.Spreadsheet(
      sheets: [
        sheets.Sheet(
          properties: sheets.SheetProperties(title: 'Clientes', sheetId: 100),
        ),
        sheets.Sheet(
          properties: sheets.SheetProperties(title: 'Debitos', sheetId: 200),
        ),
        sheets.Sheet(
          properties: sheets.SheetProperties(title: 'Parcelas', sheetId: 300),
        ),
      ],
    );
    when(
      () => mockSpreadsheets.get('test_db_id'),
    ).thenAnswer((_) async => existingSpreadsheet);

    when(() => mockValues.get('test_db_id', 'Clientes!1:1')).thenAnswer(
      (_) async => sheets.ValueRange(
        values: [
          ['id', 'nome'],
        ],
      ),
    );
    when(() => mockValues.get('test_db_id', 'Debitos!1:1')).thenAnswer(
      (_) async => sheets.ValueRange(
        values: [
          ['id', 'id_cliente', 'valor'],
        ],
    ...
    when(() => mockValues.get('test_db_id', 'Parcelas!1:1')).thenAnswer(
      (_) async => sheets.ValueRange(
        values: [
          ['id', 'id_debito', 'vencimento'],
        ],
      ),
    );

    when(
      () => mockSpreadsheets.batchUpdate(any(), 'test_db_id'),
    ).thenAnswer((_) async => sheets.BatchUpdateSpreadsheetResponse());
    when(
      () => mockValues.batchUpdate(any(), 'test_db_id'),
  ...
  test('Deve realizar exclusão em cascata (3 níveis)', () async {
    ...
    when(() => mockValues.get('test_db_id', 'Clientes!A:Z')).thenAnswer(
      (_) async => sheets.ValueRange(
        values: [
          ['id'],
          ['10'], // Linha 1
    ...
    when(() => mockValues.get('test_db_id', 'Debitos!A:Z')).thenAnswer(
      (_) async => sheets.ValueRange(
        values: [
    ...
    when(() => mockValues.get('test_db_id', 'Parcelas!A:Z')).thenAnswer(
      (_) async => sheets.ValueRange(
        values: [
    ...
    final captured = verify(
      () => mockSpreadsheets.batchUpdate(captureAny(), 'test_db_id'),
    ).captured;
    ...
    final clienteRequests = requests.where(
      (r) => r.deleteDimension?.range?.sheetId == 100,
    ); // Cliente
    final debitoRequests = requests.where(
      (r) => r.deleteDimension?.range?.sheetId == 200,
    ); // Debito
    final parcelaRequests = requests.where(
      (r) => r.deleteDimension?.range?.sheetId == 300,
    ); // Parcela
    ...
    final parcelaIndices = parcelaRequests
